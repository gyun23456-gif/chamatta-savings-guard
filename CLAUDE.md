...
- **새 탭·단계·모달을 만들면 `useBackLayer(열림여부, 닫는함수)` 로 등록한다**(`app/back.ts`). 화면 전환이 전부 상태값이라, 등록하지 않은 화면에서 토스 시스템 뒤로가기를 누르면 그 화면을 건너뛰거나 앱이 종료된다. 앱인토스 검토가 바로 이 이유로 반려된 적이 있다(`101e38a`).
- **화면 안에 ‹ 뒤로가기 버튼을 그리면 `className="app-back"` 을 붙인다.** 토스 미니앱은 내비게이션 바가 뒤로가기를 주므로, 앱 자체 버튼까지 보이면 중복이라 검토에서 반려된다(`2212feb`). `toss/main.tsx` 가 `data-in-toss` 를 붙이고 `globals.css` 가 그때만 `.app-back` 을 숨긴다(자리는 유지). 모달의 × 닫기 버튼은 해당하지 않는다.
...
루트의 `개인정보처리방침-수정안.md` 는 **이미 반영이 끝난** 검토용 초안이다(깃에 넣지 않은 로컬 파일). 한국어 본문은 `5ca5b58` 에서, 영문 사전은 같은 날 함께 개정됐다. 2026-09-12 에 방침 문장 47개를 사전과 대조해 영문 누락 0개를 확인했다. 일본어·중국어는 44개가 비어 있는데 이건 버그가 아니라 `locales-privacy.ts` 주석대로 검토 전이라 의도적으로 비워둔 것이고, 그 두 언어에서는 방침이 한국어로 보인다.
