...
// 2026-08-29 계정 기능 제거에 맞춰 개정, 같은 날 검토·승인됨.
//
...
export const enPrivacy: Dict = {
  ...
  '기기 식별값, 닉네임, 후기 제목·내용, 목표, 금액, 기간, 태그':
    'Device identifier, nickname, story title and body, goal, amount, period, tag',
  ...
  '참았다!에는 계정이 없습니다. 절약 기록과 목표, 프로필 닉네임, 즐겨찾기와 사용자 메뉴, 저축 대기금액, 은행명·계좌번호·예금주는 모두 브라우저 또는 앱의 로컬 저장공간에만 저장됩니다. 해당 정보는 참았다! 서버로 전송되지 않습니다. 앱 데이터나 브라우저 저장정보를 삭제하면 함께 삭제되므로, 기기를 옮기실 때는 설정의 기록 백업에서 파일로 내보내 옮겨주세요.':
    "Chamatta has no accounts. Your savings records and goals, profile nickname, favourites and custom menus, pending savings amounts, and your bank name, account number and account holder name are all stored only in your browser's or app's local storage. This information is not sent to Chamatta's servers. Clearing the app's data or your browser storage deletes it as well, so before moving to a new device, export a file using Backup in Settings.",
  '예외는 두 가지입니다. 첫째, 목표 달성 후기를 올리면 후기 내용과 닉네임, 기기 식별값이 서버에 저장됩니다. 둘째, 설정에서 절약 랭킹 참여를 켜면 닉네임과 기간별 절약 합계가 서버로 올라갑니다. 랭킹은 기본값이 참여하지 않음이며, 켜기 전에는 아무것도 전송되지 않습니다. 랭킹 화면에서 빠지기를 누르면 올렸던 기록이 즉시 삭제됩니다. 두 기능 모두 계정이 아닌 기기 단위로 구분하므로 같은 사람이 여러 기기를 쓰면 각각 따로 잡힙니다.':
    'There are two exceptions. First, when you post a goal story, its contents, your nickname and your device identifier are stored on our servers. Second, if you turn on leaderboard participation in Settings, your nickname and per-period savings totals are uploaded to our servers. Participation is off by default, and nothing is sent before you turn it on. Pressing Leave on the leaderboard deletes what was uploaded immediately. Both features identify you by device rather than by account, so one person using several devices is counted separately on each.',
  ...
  // 07
  '이용자의 권리': 'Your rights',
  '이용자는 자신의 개인정보에 대한 열람, 정정, 삭제 및 처리정지를 요청할 수 있습니다. 기기에만 저장된 정보는 앱 데이터 삭제 기능이나 브라우저 저장정보 삭제를 통해 직접 제거할 수 있습니다. 서버에 저장된 후기와 랭킹 기록의 삭제는 문의 이메일로 요청할 수 있습니다.':
    "You may request access to, correction of, deletion of, or a halt to the processing of your personal information. Information held only on your device can be removed directly, using the app's delete-data feature or by clearing your browser storage. To delete stories or leaderboard entries held on our servers, contact us at the email above.",
  '내 데이터 삭제 요청하기': 'Request data deletion',
