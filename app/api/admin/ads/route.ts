...
export async function GET(request: Request) {
  ...
  if (!c) return json(request, { inquiries: [], stats: [], campaigns: [], localPreview: true });
  if (!await isAdmin(request, c.secret)) return json(request, { error: '운영자 권한이 필요합니다.' }, 403);
  ...
  return json(request, { inquiries: i.results, stats: s.results, campaigns: a.results });
}

export async function POST(request: Request) {
  const c = await context();
  if (!c) return json(request, { error: '사용할 수 없습니다.' }, 503);
  if (!await isAdmin(request, c.secret)) return json(request, { error: '운영자 권한이 필요합니다.' }, 403);

  const x = await request.json() as Record<string, string>;
  if (!x.brand || !x.title || !/^https:\/\//.test(x.linkUrl ?? '')) {
    return json(request, { error: '브랜드, 제목, https 링크를 확인해주세요.' }, 400);
  }
...
export async function PATCH(request: Request) {
  const c = await context();
  if (!c) return json(request, { error: '로컬 미리보기에서는 사용할 수 없습니다.' }, 503);
  if (!await isAdmin(request, c.secret)) return json(request, { error: '운영자 권한이 필요합니다.' }, 403);

  const input = await request.json() as { id?: string; status?: string; kind?: string };
  if (!input.id) return json(request, { error: '잘못된 요청입니다.' }, 400);
  ...
  if (input.kind === 'campaign' && ['draft', 'active', 'paused'].includes(input.status ?? '')) {
    ...
    return json(request, { error: '잘못된 상태입니다.' }, 400);
  }
