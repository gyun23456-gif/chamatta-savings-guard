...
export async function GET(request: Request) {
  ...
  if (!db) return json(request, { stories: [], pending: [], localPreview: true });
  ...
  return json(request, { stories: approved.results, pending: mine.results });
}

export async function POST(request: Request) {
  const device = deviceOf(request);
  if (!device) return json(request, { error: '이 브라우저에서는 후기를 올릴 수 없어요.' }, 400);
  ...
  if (!nickname || !title || !body || !goal || !Number.isFinite(amount) || amount <= 0) {
    return json(request, { error: '필수 입력값을 확인해주세요.' }, 400);
  }

  const db = await ready();
  if (!db) return json(request, { error: '로컬 미리보기에서는 기기 저장을 사용합니다.' }, 503);
  ...
  return json(request, { id, status: 'pending', message: '후기가 등록되어 승인 대기 중입니다.' }, 201);
}

// 신고. 두 번 쌓이면 자동으로 숨긴다.
export async function PATCH(request: Request) {
  const device = deviceOf(request);
  if (!device) return json(request, { error: '이 브라우저에서는 신고할 수 없어요.' }, 400);

  const body = await request.json() as { id?: string; action?: string };
  if (!body.id || body.action !== 'report') return json(request, { error: '잘못된 요청입니다.' }, 400);

  const db = await ready();
  if (!db) return json(request, { error: '로컬 미리보기에서는 사용할 수 없습니다.' }, 503);
  ...
  return json(request, { message: '신고가 접수되었습니다.' });
}
