...
function MarketView({ cart, setCart, stories, openStory, openAdInquiry, finishOrder }: { cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; stories: Story[]; openStory: () => void; openAdInquiry: () => void; finishOrder: (result: Result, total: number, memo: string) => void }) {
  ...
  return <div className="page market-page delivery-home">
    ...
    <button className="sponsor-card" onMouseEnter={()=>fetch('/api/ads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'event',campaignId:'demo-goal-partner',eventType:'view'})})} onClick={()=>{fetch('/api/ads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'event',campaignId:'demo-goal-partner',eventType:'click'})});openAdInquiry();}}><span><em>AD · 제휴 예시</em><b>목표를 응원하는 브랜드 자리</b><small>참았다!와 함께할 파트너를 기다립니다.</small></span><i>광고 문의 ›</i></button>
...
function AdminPanel({onClose}:{onClose:()=>void}) {
  ...
  const [inquiries,setInquiries]=useState<any[]>([]); const [adStats,setAdStats]=useState<any[]>([]);
  const load=()=>{setLoading(true);Promise.all([fetch('/api/admin/stories'),fetch('/api/admin/ads')]).then(async([a,b])=>{if(!a.ok||!b.ok)throw new Error();const [p,q]=await Promise.all([a.json(),b.json()]);setStories(p.stories??[]);setInquiries(q.inquiries??[]);setAdStats(q.stats??[])}).catch(()=>setDenied(true)).finally(()=>setLoading(false));};
  useEffect(load,[]);
