...
// authenticated 는 "로그인했다"가 아니라 "이 기기에 프로필을 만들었다"는 뜻이다.
// 계정이 없어진 뒤에도 이름을 그대로 둔 건, 이 값을 읽는 자리가 여럿이라서다.
...
export default function Home() {
  ...
  const [deliveryView,setDeliveryView]=useState<'map'|'classic'>('map');
  const pickDeliveryView=(next:'map'|'classic')=>{setDeliveryView(next);try{localStorage.setItem('chamatta-delivery-view',next)}catch{/* 저장이 막혀도 이번 세션은 적용된다 */}};
  ...
  useEffect(() => {
    ...
    try { if (localStorage.getItem('chamatta-delivery-view') === 'classic') setDeliveryView('classic'); } catch { /* 기본값 그대로 간다 */ }
    // 토스 미니앱에서는 서비스워커를 걸지 않는다. 번들 갱신을 토스가 맡는데
    // 우리가 캐시까지 쥐고 있으면 옛 화면이 남는다.
    if (!IN_TOSS && 'serviceWorker' in navigator) navigator.serviceWorker.register('/sw.js').catch(() => undefined);
    try { if (!localStorage.getItem('chamatta-onboarded-v1')) setOnboarding(true); } catch { /* 저장소를 못 쓰면 건너뛴다 */ }
    setLoaded(true);
  }, []);
  // 운영자 키를 넣어둔 기기에서만 물어본다. 키가 없으면 운영자 센터는 아예 보이지 않는다.
  useEffect(() => { if (!adminKey()) return; req('/api/admin/stories', { headers: adminHeaders() }).then(r => { if (r.ok) setIsAdmin(true); }).catch(() => undefined); }, []);
  ...
  // 기기 값을 함께 보내면 내가 쓴 승인 대기 글도 같이 온다.
  useEffect(() => { const device=deviceId(); req(`/api/stories${device?`?device=${encodeURIComponent(device)}`:''}`).then(r=>r.ok?r.json() as Promise<StoriesResponse>:Promise.reject()).then(payload=>setCommunityStories([...(payload.pending??[]),...(payload.stories??[]),...demoStories])).catch(()=>setCommunityStories([...(data.stories??[]),...demoStories])); }, []);
  // 프로필은 이 기기에만 있다. 계정이 없으니 서버에서 받아올 것도 없다.
  ...
  useEffect(()=>{const local=localStorage.getItem('chamatta-local-profile');if(!local)return;try{setProfile(JSON.parse(local))}catch{localStorage.removeItem('chamatta-local-profile')}},[]);
  // 절약 기록의 서버 동기화는 없앴다. 기기 값은 localStorage 에 있어서 앱을 지우면
  // 함께 사라지고, 그러면 서버에 남은 사본을 되찾을 방법이 없다. 기기 이전은
  // 설정 > 기록 백업(파일 내보내기·불러오기)이 담당한다.
  ...
  return (
    <main className="app-shell">
      ...
      {storyOpen && <StoryModal onClose={() => setStoryOpen(false)} onSubmit={async (story) => { const optimistic:Story={...story,id:crypto.randomUUID(),createdAt:'방금 전',status:'pending'}; setCommunityStories(v=>[optimistic,...v]); setStoryOpen(false); if (energy) updateEnergy(earn(energy, REVIEW_BONUS)); try { const response=await req('/api/stories',{method:'POST',headers:{'Content-Type':'application/json',...deviceHeaders()},body:JSON.stringify(story)}); if(!response.ok) throw new Error(); } catch { setData(prev=>({...prev,stories:[optimistic,...(prev.stories??[])]})); } }} />}
      ...
      {settingsOpen && <SettingsModal deliveryView={deliveryView} setDeliveryView={pickDeliveryView} energy={energy} openEnergy={()=>{setSettingsOpen(false);setEnergyOpen(true)}} profile={profile} openProfile={()=>{setSettingsOpen(false);setProfileOpen(true)}} onClose={()=>setSettingsOpen(false)}/>} 
...
function MarketView({ cart, setCart, stories, openStory, openAdInquiry, openSettings, finishOrder, energy, spendEnergy, openEnergy, earnEnergy, nickname, deliveryView }: { deliveryView: 'map' | 'classic'; energy: Energy | null; spendEnergy: () => void; openEnergy: () => void; earnEnergy: (next: Energy) => void; nickname: string; cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; stories: Story[]; openStory: () => void; openAdInquiry: () => void; openSettings: () => void; finishOrder: (result: Result, total: number, memo: string, calories?:number) => void }) {
  ...
  if (journey) { const done=()=>{finishOrder('saved',journey.total,journey.memo,journey.calories);setCart([]);setJourney(null);setSelected(null);setOrderPaid(false);setStep('list')}; const cancel=()=>setJourney(null);
    return deliveryView==='map'
      ? <DeliveryMap amount={journey.total} calories={journey.calories} pace={pace} onComplete={done} onCancel={cancel}/>
      : <DeliveryJourney amount={journey.total} calories={journey.calories} pace={pace} onComplete={done} onCancel={cancel}/>; }
  if (step === 'shop' && selected) return <div className="page market-page"><MarketHeader title={t(selected.name)} back={() => setStep('list')} cartCount={cart.reduce((s,i)=>s+i.quantity,0)} openCart={() => setStep('cart')} /><section className="shop-hero"><span>{selected.icon}</span><div><small>{t(selected.category)} · {t(selected.time)}</small><h1>{t(selected.name)}</h1><p>★ {selected.rating} · {t('배달비')} {selected.delivery ? `${won(selected.delivery)}` : '무료'}</p></div></section><div className="shop-notice">이곳의 상점과 주문은 모두 가상 체험입니다.</div><button className="shop-review-entry" onClick={()=>setReviewShop(selected)}><span>💬</span><div><b>{t('이 가게 후기')}</b><small>{reviewLabel(selected.id)} · {t('처음 남기면 에너지 1개')}</small></div><i>›</i></button><section className="menu-list"><h2>대표 메뉴</h2>{selected.menus.map((m,i) => <Fragment key={m.id}>{m.section&&selected.menus.findIndex(x=>x.section===m.section)===i&&<div className="menu-section-title"><span>🥤</span><div><h2>{t(m.section)}</h2><small>주류는 성인용 가상 메뉴이며 실제 판매·결제되지 않아요.</small></div></div>}<article className="menu-card" data-option-profile={optionProfileForName(m.name).id}><div className="menu-copy"><h3>{t(m.name)}</h3><p>{t(m.description)}</p><small>🔥 {t('약')} {m.calories??Math.round(m.price/14)} kcal</small><b>{won(m.price)}</b><button onClick={() => setOptionMenu({menu:m,shop:selected})}>{t('옵션 선택 · 담기')}</button></div><span>{m.icon}</span></article></Fragment>)}</section>{optionMenu&&<MenuOptionsModal menu={optionMenu.menu} onClose={()=>setOptionMenu(null)} onAdd={item=>{add(item,optionMenu.shop);setOptionMenu(null)}}/>}{cart.length > 0 && <button className="floating-cart" onClick={() => setStep('cart')}><span>{cart.reduce((s,i)=>s+i.quantity,0)}</span><b>장바구니 보기</b><strong>{won(total)}</strong></button>}</div>;

  if (step === 'cart') return <div className="page market-page"><MarketHeader title="장바구니" back={() => setStep(selected ? 'shop':'list')} cartCount={0} openCart={() => undefined} />{cart.length ? <><section className="cart-shop"><small>가상 상점</small><h2>{t(cart[0].shopName)}</h2>{cart.map(i => <div className="cart-row" key={i.id}><span>{i.icon}</span><div><b>{t(i.name)}</b><small>{won(i.price)}</small></div><div className="quantity"><button onClick={() => quantity(i.id,-1)}>−</button><b>{i.quantity}</b><button onClick={() => quantity(i.id,1)}>＋</button></div></div>)}</section><section className="bill"><div><span>메뉴 금액</span><b>{won(itemTotal)}</b></div><div><span>배달비</span><b>{delivery ? `${won(delivery)}` : '무료'}</b></div><div className="bill-total"><span>총 주문금액</span><b>{won(total)}</b></div></section><button className="checkout-button" onClick={() => { if (orderPaid) return setStep('checkout'); if (!canOrder(energy)) return openEnergy(); spendEnergy(); setOrderPaid(true); setStep('checkout'); }}>{won(total)} 주문하기</button></> : <Empty icon="🛒" title="장바구니가 비었어요" text="가상 상점에서 먹고 싶은 메뉴를 골라보세요." action="상점 둘러보기" onAction={() => setStep('list')} />}</div>;

  if (step === 'checkout') return <div className="page market-page"><MarketHeader title="모의 결제" back={() => setStep('cart')} cartCount={0} openCart={() => undefined} /><div className="simulation-banner"><span>i</span><p><b>실제 결제가 아니에요</b><br/>카드·계좌 정보는 입력하거나 저장하지 않습니다.</p></div><section className="checkout-card"><h2>주문 정보</h2><div><span>{cart[0]?.shopName}</span><b>{cart.reduce((s,i)=>s+i.quantity,0)}개 메뉴</b></div><div><span>예상 열량</span><b>약 {money(totalCalories)} kcal</b></div><div><span>결제 예정 금액</span><strong>{won(total)}</strong></div></section><section className="payment-card"><h2>결제 수단 체험</h2>{['간편결제','신용·체크카드','현장 결제'].map(p => <button className={payMethod===p?'selected':''} onClick={() => setPayMethod(p)} key={p}><span>{p==='간편결제'?'⚡':p==='신용·체크카드'?'▣':'⌂'}</span><b>{p}</b><i>{payMethod===p?'●':'○'}</i></button>)}</section><section className="pace-card"><h2>마음을 식힐 시간을 골라요</h2>{['빠르게 정리','천천히 생각'].map(p=><button className={pace===p?'selected':''} key={p} onClick={()=>setPace(p)}><span>{p==='빠르게 정리'?'⚡':'🌿'}</span><div><b>{p}</b><small>{p==='빠르게 정리'?'짧은 가상 배달 체험':'조금 더 천천히 생각하기'}</small></div><i>{pace===p?'●':'○'}</i></button>)}</section><section className="decision-zone"><span>결제 직전 마지막 선택</span><h2>이 {won(total)}, 정말 쓸까요?</h2><p>어떤 선택이든 기록하면 다음 판단이 쉬워져요.</p><button className="defend-order" onClick={() => setJourney({total,calories:totalCalories,memo:`${cart[0]?.shopName} 가상 주문`})}>🛡️ 가상 배달을 시작하고 참기</button><button className="buy-order" onClick={() => { finishOrder('spent',total,`${cart[0]?.shopName ?? '가상 상점'} 가상 주문`,totalCalories); setSelected(null); setOrderPaid(false); setStep('list'); }}>모의 결제 완료 · 결국 샀다</button></section></div>;
  ...
  return <div className="page market-page delivery-home">
    ...
    <section className="shop-list">{filtered.length ? filtered.map(s => { const photos = shopPhotoSet(s); return <article className="shop-card photo-card" key={s.id} onClick={() => {setSelected(s);setStep('shop')}}>
      <div className="shop-gallery">
        <span className="shop-photo main" style={foodPhotoStyle(photos[0].index)}>{s.badge && <em>{t(s.badge)}</em>}<button className={favorites.includes(s.id)?'liked':''} onClick={e=>{e.stopPropagation();setFavorites(v=>v.includes(s.id)?v.filter(id=>id!==s.id):[...v,s.id])}} aria-label={`${t(s.name)} 찜`}>{favorites.includes(s.id)?'♥':'♡'}</button></span>
...
function MarketHeader({ title, back, cartCount, openCart }: { title: string; back?: () => void; cartCount: number; openCart: () => void }) { return <header className="market-header">{back ? <button className="app-back" onClick={back} aria-label="뒤로">‹</button> : <span className="mini-logo">ㅊ</span>}<h1>{title}</h1><button className="cart-icon" onClick={openCart} aria-label="장바구니">🛒{cartCount > 0 && <i>{cartCount}</i>}</button></header>; }
...
function HistoryView({ records, selectedDate, setSelectedDate, removeRecord, goStats }: { records: RecordItem[]; selectedDate: string; setSelectedDate: (v: string) => void; removeRecord: (id: string) => void; goStats: () => void }) {
  ...
  return <div className="page"><PageHeading eyebrow="기록 보관함" title="내역" subtitle="유혹 앞에서 했던 선택을 돌아봐요." />
    ...
    <section className="day-summary"><span>{selectedDate === dateKey() ? '오늘' : selectedDate} 지킨 돈</span><b>{won(saved)}</b><small>{filtered.length}번의 선택</small></section>
...
function StatsView({ records, allRecords, nickname, savedAmount, rate, totalSaved, savings, openSavings, goHistory }: { nickname: string; allRecords: RecordItem[]; records: RecordItem[]; savedAmount: number; rate: number; totalSaved: number; savings: SavingsQueue; openSavings: (m:'account'|'transfer')=>void; goHistory: () => void }) {
  const won = useWon();
  ...
  return <div className="page"><PageHeading eyebrow={`${new Date().getMonth() + 1}월 리포트`} title="나의 방어력" subtitle="참은 선택이 어떤 변화를 만들었는지 확인해요." />
    <section className="stat-hero"><div className="rate-ring" style={{ '--rate': `${rate * 3.6}deg` } as React.CSSProperties}><span><b>{rate}%</b><small>방어율</small></span></div><div><span>이번 달 지킨 돈</span><h2>{won(savedAmount)}</h2><p>{records.length ? `${records.length}번의 유혹 중 ${success}번을 지켰어요.` : '기록을 시작하면 분석이 쌓여요.'}</p></div></section>
    ...
    <Ranking records={allRecords} nickname={nickname} />
    <section className="section-block"><div className="section-title"><h2>실제 저축 전환</h2></div>
      <div className="transfer-card">
        <div className="transfer-top"><div><small>지금까지 지킨 돈</small><b>{won(totalSaved)}</b></div><div className="right"><small>은행으로 옮긴 돈</small><b>{won(savings.transferred)}</b></div></div>
...
function GoalsView({ goals, totalSaved, profile, savings, isAdmin, openSavings, openProfile, openSettings, openGoal, openAdmin, removeGoal }: { goals: Goal[]; totalSaved: number; profile:Profile; savings:SavingsQueue; isAdmin:boolean; openSavings:(m:'account'|'transfer')=>void; openProfile:()=>void; openSettings:()=>void; openGoal: () => void; openAdmin: () => void; removeGoal: (id: string) => void }) {
  ...
  return <div className="page"><PageHeading eyebrow="돈의 목적지" title="목표" subtitle="안 쓴 돈을 내가 원하는 미래에 연결해요." />
    ...
    <section className="savings-wallet"><div><span>🏦</span><p><small>저축 대기함</small><b>{won(savings.pending.reduce((n,p)=>n+p.amount,0))}</b><em>{savings.account?`${savings.account.bank} · ${savings.account.accountNumber.slice(-4)}`:'저축계좌를 설정해주세요'}</em></p><button onClick={()=>openSavings(savings.account?'transfer':'account')}>{savings.account?'저축하기':'계좌 설정'}</button></div><footer><span>직접 이체 완료</span><b>{won(savings.transferred)}</b><button onClick={()=>openSavings('account')}>설정</button></footer></section>
    ...
    <section className="goal-stack">{goals.length ? goals.map((g, i) => { const applied = allocations[i]; const p = Math.min(100, Math.round(applied / g.amount * 100)); return <article className="full-goal" key={g.id}><button className="delete-goal" onClick={() => removeGoal(g.id)} aria-label={`${g.name} 삭제`}>×</button><div className="goal-emoji">{g.emoji}</div><span className="eyebrow">목표 {String(i + 1).padStart(2, '0')}</span><h2>{g.name}</h2><div className="goal-big"><b>{p}%</b><span>{won(applied)} / {won(g.amount)}</span></div><div className="progress"><i style={{ width: `${p}%` }} /></div><p>{p >= 100 ? '목표 달성! 이제 다음 목적지를 정해볼까요?' : `앞으로 ${won(Math.max(0, g.amount - applied))}만 더 지키면 도착해요.`}</p></article>; }) : <Empty icon="🏁" title="아직 정한 목표가 없어요" text="지키고 싶은 돈의 목적지를 만들어보세요." action="첫 목표 만들기" onAction={openGoal} />}</section><div className="legal-links"><a href="/privacy">개인정보처리방침</a><span>·</span><a href="mailto:nomu5.help@gmail.com?subject=%EC%B0%B8%EC%95%98%EB%8B%A4!%20%EB%82%B4%20%EB%8D%B0%EC%9D%B4%ED%84%B0%20%EC%82%AD%EC%A0%9C%20%EC%9A%94%EC%B2%AD">데이터 삭제 요청</a></div>
  </div>;
}
...
function AdminPanel({onClose}:{onClose:()=>void}) {
  ...
  const load=()=>{setLoading(true);Promise.all([req('/api/admin/stories',{headers:adminHeaders()}),req('/api/admin/ads',{headers:adminHeaders()})]).then(async([a,b])=>{if(!a.ok||!b.ok)throw new Error();const [p,q]=await Promise.all([a.json() as Promise<AdminStoriesResponse>,b.json() as Promise<AdminAdsResponse>]);setStories(p.stories??[]);setInquiries(q.inquiries??[]);setAdStats(q.stats??[]);setCampaigns(q.campaigns??[])}).catch(()=>setDenied(true)).finally(()=>setLoading(false));};
...
function RecordList({ records, removable = false, onRemove }: { records: RecordItem[]; removable?: boolean; onRemove?: (id: string) => void }) {
  const won = useWon(); return <div className="record-list">{records.map(item => { const cat = getCategory(item.category); return <div className="record" key={item.id}><span className="record-icon" style={{ background: cat.color }}>{cat.icon}</span><div><b>{item.memo || item.category}</b><small>{item.category} · {item.date}</small></div><strong className={item.result === 'spent' ? 'spent' : ''}>{item.result === 'saved' ? '+' : '−'}{won(item.amount)}</strong>{removable && <button className="remove-record" onClick={() => onRemove?.(item.id)} aria-label="기록 삭제">×</button>}</div>; })}</div>; }
...
function SuccessModal({ record, total, streak, onQueue, onClose }: { record: RecordItem; total: number; streak: number; onQueue:()=>void; onClose: () => void }) {
  const won = useWon(); return <div className="success-screen" role="dialog" aria-modal="true"><div className="confetti">✦ <i>●</i> ✦ <em>◆</em> ✦</div><div className="success-shield"><span>✓</span></div><span className="success-label">방어 성공</span><h2>{won(record.amount)}</h2><p>{record.memo || record.category}의 유혹을 넘겼어요.<br/>{record.calories?`예상 ${money(record.calories)} kcal도 함께 피했어요.`:'오늘의 선택이 미래의 나를 만들어요.'}</p><div className="success-stats"><div><small>누적 지킨 돈</small><b>{won(total)}</b></div><i/><div><small>연속 방어</small><b>🔥 {streak}일</b></div></div><button className="queue-saving" onClick={onQueue}>🏦 이 금액, 진짜 저축하기</button><button className="success-later" onClick={onClose}>나중에 할게요</button></div>; }
