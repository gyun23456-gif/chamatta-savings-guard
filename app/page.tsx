...
export default function Home() {
  ...
  useEffect(() => { if (!adminKey()) return; fetch('/api/admin/stories', { headers: adminHeaders() }).then(r => { if (r.ok) setIsAdmin(true); }).catch(() => undefined); }, []);
  ...
  useEffect(() => { const device=deviceId(); fetch(`/api/stories${device?`?device=${encodeURIComponent(device)}`:''}`).then(r=>r.ok?r.json() as Promise<StoriesResponse>:Promise.reject()).then(payload=>setCommunityStories([...(payload.pending??[]),...(payload.stories??[]),...demoStories])).catch(()=>setCommunityStories([...(data.stories??[]),...demoStories])); }, []);
...
function AdminPanel({onClose}:{onClose:()=>void}) {
  ...
  const load=()=>{setLoading(true);Promise.all([fetch('/api/admin/stories',{headers:adminHeaders()}),fetch('/api/admin/ads',{headers:adminHeaders()})]).then(async([a,b])=>{if(!a.ok||!b.ok)throw new Error();const [p,q]=await Promise.all([a.json() as Promise<AdminStoriesResponse>,b.json() as Promise<AdminAdsResponse>]);setStories(p.stories??[]);setInquiries(q.inquiries??[]);setAdStats(q.stats??[]);setCampaigns(q.campaigns??[])}).catch(()=>setDenied(true)).finally(()=>setLoading(false));};
  ...
  useEffect(load,[]);
  const act=async(id:string,action:string)=>{await fetch('/api/admin/stories',{method:'PATCH',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({id,action})});load();};
  const adAct=async(id:string,status:string)=>{await fetch('/api/admin/ads',{method:'PATCH',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({id,status})});load();};
  const campaignAct=async(id:string,status:string)=>{await fetch('/api/admin/ads',{method:'PATCH',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({id,status,kind:'campaign'})});load()};
