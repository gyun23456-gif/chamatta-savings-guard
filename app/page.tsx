...
function MarketView({ cart, setCart, stories, openStory, openAdInquiry, openSettings, finishOrder, energy, spendEnergy, openEnergy, deliveryView }: { deliveryView: 'map' | 'classic'; energy: Energy | null; spendEnergy: () => void; openEnergy: () => void; cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; stories: Story[]; openStory: () => void; openAdInquiry: () => void; openSettings: () => void; finishOrder: (result: Result, total: number, memo: string, calories?:number) => void }) {
  ...
  return <div className="page market-page delivery-home">
    ...
    <section className="popular-section"><div className="popular-heading"><div><span>TODAY&apos;S POPULAR</span><h2>지금 많이 참는 메뉴</h2><p>가상 주문에서 가장 자주 선택된 메뉴예요.</p></div></div><div className="popular-scroll">{popular.map(({shop,menu,rank})=><button key={shop.id} onClick={()=>{setSelected(shop);setStep('shop')}}><span className="popular-photo" style={foodPhotoStyle(shopPhotoIndex(shop))}><i>{rank}</i></span><b>{t(menu.name)}</b><small>{t(shop.name)}</small><em>{money(menu.price)}원</em></button>)}</div></section>
    ...
    <section className="shop-list">{filtered.length ? filtered.map(s => { const photos = shopPhotoSet(s); return <article className="shop-card photo-card" key={s.id} onClick={() => {setSelected(s);setStep('shop')}}>
      ...
      <div className="shop-card-copy"><div><h2>{t(s.name)}</h2><strong>{t(s.time)}</strong></div><p><b>★ {s.rating}</b> · {money(reviewCount(s))} {t('리뷰')} <i>›</i></p><footer><span>{t(s.category)}</span><small>{t('배달비')} {s.delivery ? `${money(s.delivery)}원` : t('무료')}</small></footer></div>
