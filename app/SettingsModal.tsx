...
export default function SettingsModal({profile,openProfile,onClose,energy,openEnergy,deliveryView,setDeliveryView}:{deliveryView:'map'|'classic';setDeliveryView:(v:'map'|'classic')=>void;energy:EnergyView|null;openEnergy:()=>void;profile:Profile;openProfile:()=>void;onClose:()=>void}){
  ...
  return <div className="modal-backdrop"><section className="modal-sheet settings-sheet"><header><div><span>MY SETTINGS</span><h2>설정</h2><p>참았다!를 내 방식에 맞게 사용해요.</p></div><button onClick={onClose}>×</button></header>
    ...
    <section className="settings-group"><h3>{t('테마')}</h3><p>{t('앱의 대표 색상을 골라보세요.')}</p><div className="theme-grid"><button className={theme==='red'?'selected':''} onClick={()=>setTheme('red')}><span className="red-swatch"/><b>{t('레드')}</b>{theme==='red'&&<i>✓</i>}</button><button className={theme==='orange'?'selected':''} onClick={()=>setTheme('orange')}><span className="orange-swatch"/><b>{t('오렌지')}</b>{theme==='orange'&&<i>✓</i>}</button><button className={theme==='forest'?'selected':''} onClick={()=>setTheme('forest')}><span className="forest-swatch"/><b>{t('포레스트')}</b>{theme==='forest'&&<i>✓</i>}</button></div></section>

    <section className="settings-group"><h3>{t('배달 화면')}</h3><p>{t('가상 배달이 오는 동안 보여줄 화면을 골라요.')}</p><div className="theme-grid"><button className={deliveryView==='map'?'selected':''} onClick={()=>setDeliveryView('map')}><span>🗺️</span><b>{t('지도 배달')}</b>{deliveryView==='map'&&<i>✓</i>}</button><button className={deliveryView==='classic'?'selected':''} onClick={()=>setDeliveryView('classic')}><span>🛵</span><b>{t('기존 맵')}</b>{deliveryView==='classic'&&<i>✓</i>}</button></div><p className="settings-fineprint">지도는 가상의 동네를 그린 그림이에요. 실제 위치를 쓰지 않고, 위치 권한도 요구하지 않습니다.</p></section>

    <section className="settings-group"><h3>{t('가상 주문 에너지')}</h3><p>가상 주문 1번에 에너지 3개를 씁니다. 앱을 켠 날마다 3개씩 지급돼요.</p>
    ...
    <section className="settings-group"><h3>{t('기록 백업')}</h3><p>기록은 이 기기에만 저장돼요. 폰을 바꾸거나 앱을 다시 깔기 전에 꼭 내보내두세요.</p>
    ...
    <section className="settings-group"><h3>{t('내 데이터 삭제')}</h3><p>언제든 직접 지울 수 있어요. 되돌릴 수 없습니다.</p>
      {wiped
